import { createInitialStockAction, createTransferAction } from "@/app/actions";
...
export default async function WarehousePage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  const [{ data: balances }, { data: movements }, { data: companies }, { data: products }, { data: warehouses }] = await Promise.all([
    supabase.from("v_stock_balance").select("company_id,product_id,warehouse_id,unit,quantity,companies(name),products(commercial_name)").order("company_id"),
