...
export default async function QdcaPage({ searchParams }: { searchParams: Promise<{ company?: string; from?: string; to?: string }> }) {
  ...
  return <>
    <div className="qdca-banner"><div><p className="eyebrow">REGISTRO ELETTRONICO</p><h1 style={{margin:0}}>Quaderno di campagna digitale</h1><p>Registro machine-readable con localizzazione, superficie, EPPO/BBCH, prodotto e autorizzazione, quantità e dose normalizzata in L/ha o kg/ha quando applicabile.</p></div><a className="btn accent" href={`/api/export/qdca?${exportParams.toString()}`}>↓ Esporta CSV</a></div>
    <PageHeader title="Registro trattamenti" description="I dati derivano dalle esecuzioni completate e conservano snapshot storici. L'export include anche unità geospaziale/domanda aiuto e dose standardizzata per ettaro." />
