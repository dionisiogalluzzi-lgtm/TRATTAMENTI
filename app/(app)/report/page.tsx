...
export default async function ReportsPage() {
  ...
  const [{data:costRows},{data:qdca},{data:activities}]=await Promise.all([
    supabase.from("v_cost_by_field").select("company_id,field_id,field_name,total_cost,companies(name)"),
    supabase.from("v_qdca_records").select("company_id,company_name,field_id,field_name,product_id,product_name,quantity_used,unit,treated_area_ha,execution_date"),
    supabase.from("activities").select("activity_type,direct_cost,company_id,companies(name)")
  ]);
  const byField=new Map<string,{field:string;company:string;cost:number}>(); for(const r of costRows??[]){const key=String(r.field_id);const x=byField.get(key)||{field:r.field_name??"—",company:(r as any).companies?.name??"—",cost:0};x.cost+=Number(r.total_cost??0);byField.set(key,x)}
  const byProduct=new Map<string,{name:string;quantity:number;unit:string;applications:Set<string>}>(); for(const r of qdca??[]){const key=String(r.product_id);const x=byProduct.get(key)||{name:r.product_name??"—",quantity:0,unit:r.unit??"",applications:new Set<string>()};x.quantity+=Number(r.quantity_used??0);x.applications.add(`${r.execution_date}-${r.field_id}`);byProduct.set(key,x)}
  const activityCost=(activities??[]).reduce((s:number,a:any)=>s+Number(a.direct_cost??0),0);
  const treatedArea=(qdca??[]).reduce((s:number,r:any)=>s+Number(r.treated_area_ha??0),0);
  return <>
