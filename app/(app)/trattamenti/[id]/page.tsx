...
import { refreshPlanFullAction } from "@/app/compliance-actions";
...
export default async function TreatmentDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  const canStart = blockingCount === 0 && ["PRONTO","PIANIFICATO"].includes((plan as any).status) && (profile.role === "ADMIN" || (plan as any).assigned_operator_id === userId);
  return <>
    ...
    <div className="detail-grid">
      <div className="stack">
        ...
        <section className="panel"><div className="section-head"><div><p className="eyebrow">CONTROLLO ETICHETTA</p><h2>Conformità del piano</h2></div>{blockingCount?<span className="badge danger">BLOCCATO</span>:warningCount?<span className="badge warning">DA VERIFICARE</span>:<span className="badge success">NESSUN BLOCCO</span>}</div>
          <p className="muted">Il motore confronta i dati censiti dell'etichetta con coltura, dose, intervallo minimo e numero applicazioni. Se l'etichetta non è ancora censita, genera un avviso e richiede verifica manuale.</p>
      ...
      <div className="stack">
        ...
        {!canStart && blockingCount>0 && <div className="alert error">L'esecuzione è disabilitata finché esistono controlli BLOCKING.</div>}
      </div>
    </div>
  </>;
}
