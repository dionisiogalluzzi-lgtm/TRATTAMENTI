...
export async function GET(request: Request) {
  ...
  const headers = [
    ...
    "intervallo_carenza_giorni","operatore","attrezzatura","temperatura_c","umidita_pct","vento_kmh","note"
  ];
  ...
  for (const r of data ?? []) {
    lines.push([
      ...
      r.preharvest_interval_days,r.operator_name,r.equipment_name,r.weather_temperature_c,r.weather_humidity_pct,r.weather_wind_kmh,r.notes,
    ].map(cell).join(";"));
  }
